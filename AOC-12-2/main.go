...
func getPaths(path []string, connections map[string][]string, duplicate bool) [][]string {
	...
	for _, nxt := range connections[path[len(path)-1]] {
		if nxt == "end" {
			...
			options = append(options, getPaths(append(path, nxt), connections, duplicate)...)
		} else if !duplicate && nxt != "start" {
			options = append(options, getPaths(append(path, nxt), connections, true)...)
		}
	}
...
func main() {
	...
	paths := getPaths([]string{"start"}, connections, false)
